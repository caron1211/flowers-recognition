...
# path_test = "C:/Users/caron/Desktop/flower recognetion/myFlower/"
path_test = "input/test/test"
batch_size = 15


def trainModel():
    train_df, validate_df = PrepareData.prepareTrain(path_train)
    PrepareData.showDivision(train_df)
...
def testModel(model):
    ...
    test_df = PrepareData.prepareTest(path_test)
    nb_samples = test_df.shape[0]

    train_df, _ = PrepareData.prepareTrain(path_train)
    ...
    y_true = PrepareData.convertToBinary(test_df['category'])
    ...
    test_df['category'] = test_df['category'].replace({'flower': 1, 'noFlower': 0})

    PrepareData.showDivision(test_df)
